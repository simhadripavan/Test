...
RELEASE_VERSION_LAST_SEEN=$(grep ${PROPERTY_VERSION_BUILD_LAST_SEEN} ${SYSTEM_UPGRADE_SUPPORT_PROPERTIES_FILE}| tr -d "\r\n"| awk -F "=" '{print $2}')
...
check_if_release_version_changed() {
    ...
    if [ "${RELEASE_VERSION_LAST_SEEN}" == "${RELEASE_VERSION_CURRENT}" ]; then
        ...
        echo "Release version did not change for System. No action taken."
        exit 0
    else
